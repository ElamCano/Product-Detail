...
export default function Hero() {
  ...
  useEffect(() => {
    ...
    const fetchData = async (props) => {
      try {
        const requestJson = await fetch("/Products.json");
  ...
  if (!product) {
    return (
      <div className="absolute text-[100px] top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%]">
        <div className="loader ease-linear rounded-full border-4 border-t-4 border-gray-700 h-48 w-48"></div>
      </div>
    );
  }
  ...
  return (
    <section className="flex flex-col md:flex-row h-screen">
      <div className="flex relative h-[70%] md:h-screen w-full md:w-[55%]">
        <div className="flex-1 bg-red-700"></div>
        <img
          src="/red_shirt_front.png"
          alt=""
          className="absolute z-10 w-[450px] lg:w-[500px] max-w-full max-h-full top-[50%] left-[50%] translate-y-[-40%] translate-x-[-50%]"
        />
